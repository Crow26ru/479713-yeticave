...
if(isset($_SESSION['user']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Проверяем дату на формат
    if(!check_date_format($lot['lot-date'])) {
        $errors['lot-date'] = 'Укажите дату завершения торгов в формате ДД.ММ.ГГГГ';
    };
        
    // Проверяем дату окончания лота, что указанное значение не меньше следующего дня
    $lot['lot-date'] = strtotime($lot['lot-date']);
    $date_end = strtotime('tomorrow');

    if($lot['lot-date'] <= $date_end) {
        $errors['lot-date'] = 'Дата завершения торгов должна быть больше минимум на день, чем текущая дата';
    }
    ...
    if(!$errors) {
        ...
        // Обработка времени
        $lot['lot-date'] = date('Y-m-d H:i:s', $lot['lot-date']);
